...
void Eeprom::LoadBlocking() {
    ...
    hexdump(ptr + 2, EEPROM_PAGE_SIZE);

    printf("PPM  : %u\n", +m_data->page.ppm_target);
    printf("SSID : %.*s\n", NETWORK_SSID_MAX_LENGTH, m_data->page.ssid);
    printf("PASS : %.*s\n", NETWORK_PASSWORD_MAX_LENGTH, m_data->page.password);
...
void Eeprom::task()
{
    while (true) {
        ...
        printf("EEPROM flush requested\n");
        std::lock_guard<Fmutex> flush_guard { m_flush_mutex };
        {
            std::lock_guard<Fmutex> access_guard { m_access_mutex };
            if (!eeprom_should_flush(*m_data)) {
                printf("EEPROM up-to-date\n");
                continue;
            }
            printf("EEPROM out-of-date: flushing...\n");
            ...
            printf("New crc:0x%04x\n", +crc);
            m_data->page.checksum = crc << 8 | crc >> 8;
            printf("(checksum):0x%04x\n", +m_data->page.checksum);
            hexdump(ptr + 2, EEPROM_PAGE_SIZE);
            ...
            printf("Successfully flushed to EEPROM\n");
        }
